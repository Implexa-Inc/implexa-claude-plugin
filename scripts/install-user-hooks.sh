...
#   4. Wires the Implexa MCP server into Claude Desktop / Cowork
...
# Holds your API key + API URL. This is the canonical on-disk config; the
# launchctl env (section 6b) and Claude Desktop MCP wiring (section 6c) carry
# the same values to GUI-launched Claude processes so the plugin's background
# hooks + MCP server can reach the backend.
...
if [ -z "$PLUGIN_HOOK" ]; then
  ...
  echo ""
  echo "    The config + MCP wiring are in place and will activate as soon as the plugin is."
