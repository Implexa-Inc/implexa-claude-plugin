...
if [ -n "$runs" ]; then
  CTX="${CTX} RUN requests (run an existing agent): ${runs}. For each: call list_scheduled_skills and match by skillSlug to get its scheduled-skill id + timezone. Then FIRE it on the native runtime (which has full capability - computer-use, the logged-in browser, local files) instead of running inline: create_scheduled_task with taskId '<skillSlug>-now', prompt '/implexa:run-scheduled <its scheduled-skill id> --on-demand', and fireAt = now + 1 minute as an ISO timestamp with timezone offset (NO cron; fires once and auto-disables) - or, if a task named '<skillSlug>-now' already exists, update_scheduled_task with the new fireAt to re-arm it. The --on-demand marker makes it run even if the routine is PAUSED, without un-pausing it (an explicit run-now always works; pause only stops the clock). Then resolve_run_request with status 'done' right away and tell the user the agent is firing in the background within a minute or two and its result lands in their Implexa inbox. ONLY if the agent has no scheduled-skill at all (never registered), fall back to running it inline with apply_workflow, then IMMEDIATELY resolve_run_request with status \"done\" (and run_id). Never skip that close call - it records the run + clears it from the desktop."
fi
if [ -n "$activates" ]; then
